(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(document).on(
      ...
      function () {
        $(".accordion-item-wrapp .accordion-item-content").removeClass(
          "active",
        );
        $(".according-img-tab").hide();

        var activeTab = $(this)
          .find(".accordion-tab-item")
          .attr("data-bs-target");
        $(activeTab).fadeIn();
        return false;
      },
  ...
  function popupSideMenu($sideMenu, $sideMenuOpen, $sideMenuCls, $toggleCls) {
    ...
    $(document).on("click", function (e) {
      if (
        ...
        !$(e.target).closest($sideMenuOpen).length
      ) {
    ...
    $(document).on("click", "#loadMore", function (e) {
      ...
      if ($(".loadcontent:hidden").length === 0) {
        $("#loadMore")
          .text("No Content")
          .addClass("noContent")
          .prop("disabled", true); // Optional: disable button when no content is left
      }
    });
  });
